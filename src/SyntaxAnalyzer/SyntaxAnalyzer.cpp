	...
	else
	{
		...
		if (lexeme.getToken() == Token::OPEN_BRACKET)
		{
			...
			do
			{
				...
				if (lexeme.getToken() != Token::DELIMITER && lexeme.getToken() != Token::CLOSE_BRACKET)
				{
...
std::unique_ptr<ExpressionAst> thl::SyntaxAnalyzer::parseExpression(Lexeme lexeme)
{
	...
	if (lhs)
	{
		...
		if ((getToken == Token::IMPLICATION) || (getToken == Token::IMPLICATIONB))
		{
			...
			if (rhs)
			{
				result = std::make_unique<BinaryExprAST>(getToken, std::move(lhs), std::move(rhs));
			}
		}
...
std::unique_ptr<ExpressionAst> thl::SyntaxAnalyzer::parseImplExpression(Lexeme lexeme)
{
	std::unique_ptr<ExpressionAst> result = nullptr;
	...
	if (lhs)
	{
		lexeme = getLexeme();
		Token getToken = lexeme.getToken();
		if ((getToken == Token::ADD) || (getToken == Token::SUB) || (getToken == Token::OR) || (getToken == Token::XOR))
		{
			lexeme = getLexeme();
...
std::unique_ptr<ExpressionAst> thl::SyntaxAnalyzer::parseParenExpr(Lexeme lexeme)
{
	std::unique_ptr<ExpressionAst> result = nullptr;
...
{
	auto result = std::make_unique<NumberExprAST>((*m_constTable)[lexeme.getAttribute()]);
